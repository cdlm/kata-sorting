...
public class IntegerSequence {
	...
	/**
	 * Construit une séquence <i>vide</i>.
	 * 
	 * @param capacity
	 *            Nombre maximum d'éléments pouvant être ajoutés dans la
	 *            séquence.
	 */
	...
	/** @return Taille effective de la séquence (nombre d'éléments). */
	public int size() {
		return size;
	}

	/** @return Espace total disponible (nombre maximum d'éléments). */
	public int capacity() {
	...
	/** Trie la séquence par tri-fusion. */
	public void mergeSort() {
		tmp = new int[storage.length];
		// {{{ task implémenter le tri fusion
		mergeSort(0, size - 1);
		// }}}
		tmp = null; // tmp is now wasted space, throw it away
	}

	/** Trie la séquence par tri-insertion. */
	public void insertSort() {
		// {{{ task implémenter le tri insertion
		insertSort(size);
		// }}}
	}
	...
	/*
	 * Les méthodes suivantes sont fournies pour faciliter la construction des
	 * données lors des mesures. Voir <code>IntegerSequenceTest</code> pour des
	...
	public void shuffle(long seed) {
		...
		for (int index = storage.length - 1; index > 0; index--) {
			// other takes a value in 0..index
	...
	/**
	 * Échange deux éléments de la séquence.
	 * 
	 * @param index Indice d'un élément.
	 * @param other Indice de l'autre élément.
	 */
	 ...
	 * 
	 * @return <code>true</code> si les éléments sont en ordre croissant.
	 */
